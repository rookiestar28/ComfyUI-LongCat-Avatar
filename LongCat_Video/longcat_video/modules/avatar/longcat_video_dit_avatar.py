...
class LongCatAvatarSingleStreamBlock(nn.Module):
    def __init__(
        ...
        self.attn = Attention(
            ...
            enable_sageattn3=enable_sageattn3,
...
class LongCatVideoAvatarTransformer3DModel(
    ...
    def __init__(
        ...
        self.lora_dict = {}
        self.active_loras = []
        self.lora_runtime_offload = True

    @staticmethod
    def _module_needs_move(module: nn.Module, target_device: torch.device, target_dtype: torch.dtype) -> bool:
        for tensor in module.parameters(recurse=True):
            return tensor.device != target_device or tensor.dtype != target_dtype
    ...
    def _create_multi_lora_forward(self, module, loras):
        def multi_lora_forward(x, *args, **kwargs):
            ...
            offload_after_forward = bool(getattr(self, "lora_runtime_offload", True))
            ...
            for lora in loras:
                if lora.use_lora:
                    if self._module_needs_move(lora.lora_down, target_device, weight_dtype):
                        lora.lora_down.to(target_device, dtype=weight_dtype, non_blocking=True)
                    if self._module_needs_move(lora.lora_up, target_device, weight_dtype):
                    ...
                    # CRITICAL: block_num=0 is the full-load path; do not offload LoRA per forward or PCIe transfers starve the GPU.
