...
class LongCatVideoAvatarPipeline:
    ...
    def _cache_clean_latents(self, cond_latents, model_max_length, offload_kv_cache, device, dtype, audio_embs, num_cond_latents, num_ref_latents, ref_img_index):
        ...
        # CRITICAL: _model_ctx(None) returns nullcontext(self.dit); the DiT module itself is not a context manager.
        model_context = self._model_ctx(self.streaming_prefetch_count)
    ...
    def generate_at2v(
        ...
        with debug_profile.phase("vae_offload_before_denoise"):
            self.vae_offload()
        # CRITICAL: _model_ctx(None) returns nullcontext(self.dit); the DiT module itself is not a context manager.
        model_context = self._model_ctx(self.streaming_prefetch_count)
        with debug_profile.phase("denoising_loop", steps=len(timesteps), guidance=self.do_classifier_free_guidance):
            with model_context as self.dit:
        ...
        with debug_profile.phase("vae_decode"):
            latents = latents.to(self.vae.dtype)
            latents = self.denormalize_latents(latents)
            output_video = self.vae.decode(latents, return_dict=False)[0]
        with debug_profile.phase("postprocess_video"):
    ...
    def generate_ai2v(
        ...
        # CRITICAL: _model_ctx(None) returns nullcontext(self.dit); the DiT module itself is not a context manager.
        model_context = self._model_ctx(self.streaming_prefetch_count)
        debug_profile.start_phase("denoising_loop", steps=len(timesteps), guidance=self.do_classifier_free_guidance)
        with model_context as self.dit:
            with tqdm(total=len(timesteps), desc="Denoising") as progress_bar:
                for i, t in enumerate(timesteps):
                    if self.interrupt:
    ...
    def generate_avc(
        ...
        if use_kv_cache:
            with debug_profile.phase("kv_cache_prepare", offload_kv_cache=offload_kv_cache):
